...
class GcpKmsFilter:
    def run(self, method, **kwargs):
        params = {
            ...
            "additional_authenticated_data": kwargs.get(
    ...
    def kms_decrypt(self, module):
        ...
        if module.params["additional_authenticated_data"]:
            payload["additionalAuthenticatedData"] = module.params[
                "additional_authenticated_data"
            ]

        auth = GcpSession(module, "cloudkms")
        url = (
            "https://cloudkms.googleapis.com/v1/projects/{projects}/locations/{locations}/"
            "keyRings/{key_ring}/cryptoKeys/{crypto_key}:decrypt".format(
                **module.params
            )
        )
    ...
    def kms_encrypt(self, module):
        ...
        url = (
            "https://cloudkms.googleapis.com/v1/projects/{projects}/locations/{locations}/"
            "keyRings/{key_ring}/cryptoKeys/{crypto_key}:encrypt".format(
                **module.params
            )
        )
        response = auth.post(url, body=payload)
...
class FilterModule(object):
    def filters(self):
        return {"gcp_kms_encrypt": gcp_kms_encrypt, "gcp_kms_decrypt": gcp_kms_decrypt}
