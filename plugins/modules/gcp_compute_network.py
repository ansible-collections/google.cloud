...
def main():
    ...
    module = GcpModule(
        argument_spec=dict(
            ...
            routing_config=dict(
                type="dict", options=dict(routing_mode=dict(required=True, type="str"))
            ),
            mtu=dict(type="int"),
        )
    )
    ...
    auth.patch(
        ...
        ).format(**module.params),
        {
            "routingConfig": NetworkRoutingconfig(
                module.params.get("routing_config", {}), module
            ).to_request()
        },
    )
...
def resource_to_request(module):
    request = {
        ...
        "autoCreateSubnetworks": module.params.get("auto_create_subnetworks"),
        "routingConfig": NetworkRoutingconfig(
            module.params.get("routing_config", {}), module
        ).to_request(),
...
def self_link(module):
    return "https://compute.googleapis.com/compute/v1/projects/{project}/global/networks/{name}".format(
        **module.params
    )


def collection(module):
    return "https://compute.googleapis.com/compute/v1/projects/{project}/global/networks".format(
        **module.params
    )
...
def response_to_hash(module, response):
    return {
        "description": module.params.get("description"),
        "gatewayIPv4": response.get("gatewayIPv4"),
        "id": response.get("id"),
        "name": module.params.get("name"),
        ...
        "routingConfig": NetworkRoutingconfig(
            response.get("routingConfig", {}), module
        ).from_response(),
        "mtu": module.params.get("mtu"),
    }
...
def wait_for_operation(module, response):
    ...
    return fetch_resource(
        module, navigate_hash(wait_done, ["targetLink"]), "compute#network"
    )
...
class NetworkRoutingconfig(object):
    ...
    def to_request(self):
        return remove_nones_from_dict({"routingMode": self.request.get("routing_mode")})

    def from_response(self):
        return remove_nones_from_dict({"routingMode": self.request.get("routingMode")})
