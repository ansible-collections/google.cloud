    ...
    module = GcpModule(
        argument_spec=dict(
            ...
            interfaces=dict(
                type="list",
                elements="dict",
                options=dict(id=dict(type="int"), ip_address=dict(type="str")),
            ),
        )
    )
...
def resource_to_request(module):
    request = {
        ...
        "redundancyType": module.params.get("redundancy_type"),
        "interfaces": ExternalVpnGatewayInterfacesArray(
            module.params.get("interfaces", []), module
        ).to_request(),
    }
    return_vals = {}
...
def self_link(module):
    return "https://compute.googleapis.com/compute/v1/projects/{project}/global/externalVpnGateways/{name}".format(
        **module.params
    )


def collection(module):
    return "https://compute.googleapis.com/compute/v1/projects/{project}/global/externalVpnGateways".format(
        **module.params
    )
...
def response_to_hash(module, response):
    return {
        ...
        "redundancyType": response.get("redundancyType"),
        "interfaces": ExternalVpnGatewayInterfacesArray(
            response.get("interfaces", []), module
        ).from_response(),
    }
...
def wait_for_operation(module, response):
    ...
    return fetch_resource(
        module, navigate_hash(wait_done, ["targetLink"]), "compute#externalVpnGateway"
    )
...
class ExternalVpnGatewayInterfacesArray(object):
    ...
    def _request_for_item(self, item):
        return remove_nones_from_dict(
            {"id": item.get("id"), "ipAddress": item.get("ip_address")}
        )

    def _response_from_item(self, item):
        return remove_nones_from_dict(
            {"id": item.get("id"), "ipAddress": item.get("ipAddress")}
        )
